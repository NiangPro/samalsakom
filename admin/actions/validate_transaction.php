...
try {
    ...
    if ($transaction['statut'] !== 'pending') {
        throw new Exception('Cette transaction ne peut pas être validée');
    }

    // Mettre à jour le statut
    $update_query = "UPDATE cotisations SET statut = 'completed', date_validation = NOW() WHERE id = ?";
    $stmt = $db->prepare($update_query);
    $stmt->execute([$transaction_id]);

    // Créer une notification pour l'utilisateur
    $notification_message = "Votre {$transaction['type_transaction']} de {$transaction['montant']} FCFA pour la tontine {$transaction['tontine_nom']} a été validée";
    
    $notif_query = "INSERT INTO notifications (user_id, type, titre, message, date_creation) 
                    VALUES (?, 'validation', 'Transaction validée', ?, NOW())";
    ...
    try {
        $log_query = "INSERT INTO admin_logs (admin_id, action, details, date_creation) 
                      VALUES (?, 'validate_transaction', ?, NOW())";
        $stmt = $db->prepare($log_query);
        $stmt->execute([
            $_SESSION['admin_id'],
            "Transaction validée: #{$transaction_id} - {$transaction['type_transaction']} de {$transaction['montant']} FCFA pour {$transaction['prenom']} {$transaction['nom']}"
        ]);
    ...
    error_log("Erreur validate_transaction: " . $e->getMessage());
    echo json_encode([
        'success' => false,
